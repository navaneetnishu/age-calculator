...
const firstNames = [
  ...
  'Bailey', 'Brooklyn', 'Carson', 'Charlie', 'Dakota', 'Dallas', 'Dana', 'Drew', 'Ellis', 'Emery'
];
...
const categories = [
  'actor', 'actress', 'musician', 'athlete', 'entrepreneur', 'internet-celebrity', 'scientist', 'politician'
];
...
function generateCelebrity(id, category) {
  ...
  return {
    ...
    profession,
    nationality,
    bio: `${name} is a ${nationality} ${profession.toLowerCase()} known for their work in the entertainment industry.`,
    zodiacSign,
    socialMedia: {
    ...
    achievements: [
      ...
      `Notable work in ${category} industry`
    ],
    isActive: Math.random() > 0.1 // 90% active
  };
}

// Generate celebrities for a category
function generateCelebritiesForCategory(category, count) {
  const celebrities = [];
  for (let i = 0; i < count; i++) {
    celebrities.push(generateCelebrity(i + 1, category));
  }
  return celebrities;
}

// Main function to generate all celebrity data
function generateAllCelebrities() {
  const celebritiesDir = path.join(__dirname, '..', 'src', 'data', 'celebrities');
  
  // Ensure directory exists
  if (!fs.existsSync(celebritiesDir)) {
    fs.mkdirSync(celebritiesDir, { recursive: true });
  }

  const categories = [
    { id: 'actors', label: 'Actors & Actresses', icon: '🎭', count: 200 },
    { id: 'musicians', label: 'Musicians & Singers', icon: '🎤', count: 150 },
    { id: 'athletes', label: 'Athletes & Sports', icon: '⚽', count: 100 },
    { id: 'entrepreneurs', label: 'Entrepreneurs', icon: '💼', count: 80 },
    { id: 'internet-celebrities', label: 'Internet Celebrities', icon: '📱', count: 120 },
    { id: 'scientists', label: 'Scientists & Intellectuals', icon: '🔬', count: 60 },
    { id: 'politicians', label: 'Politicians & Public Figures', icon: '🏛️', count: 40 }
  ];
  ...
  categories.forEach(category => {
    ...
    const celebrities = generateCelebritiesForCategory(category.id, category.count);
...
if (require.main === module) {
  console.log('🚀 Starting celebrity data generation...\n');
  generateAllCelebrities();
}

module.exports = { generateAllCelebrities };
